...
for i in range(len(list_log)):
	...
	if list_log[i][-1]!= "-" and ip_adress in list_ipadress_timeout:
		...
		if list_times_timeout[index_number_timeout] >= N:
			print("サーバー復帰　"+ip_adress+"　: "+list_reftime_timeout[index_number_timeout]+"~"+str(float(ref_time)-float(list_reftime_timeout[index_number_timeout])+float(int(ping)*0.001))+"(S)")
		...
		if ip_adress in list_ipadress_timeout_real:
			...
			for l in list_swich:
				...
				if not set(list_ipadress_server[list_swich.index(l)]).issubset(list_ipadress_timeout_real) and list_timeout_server[list_swich.index(l)]!= 0:
					...
					print("スイッチ復帰　"+l+" : "+list_timeout_server[list_swich.index(l)]+"~"+str(float(ref_time)-float(list_timeout_server[list_swich.index(l)])+float(int(ping)*0.001))+"(S)")
	...
	if "-" not in list_ping_average[index_number_ping] and len(list_ping_average[index_number_ping]) == m:
		...
		#過負荷状態が解けた時
			...
			print("過負荷解消　"+list_ping[index_number_ping]+ " : "+list_ping_weight_time[index_number_ping_time]+"~"+str(float(ref_time)-float(list_ping_weight_time[index_number_ping_time])+float(int(ping)*0.001))+"(S)")
